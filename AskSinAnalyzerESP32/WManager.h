...
static bool deInitBLE(void) {
  esp_err_t status;// = esp_ble_gatts_app_unregister(app_gatts_if);
  if (status != ESP_OK) {
    Serial.print("esp_ble_gatts_app_unregister status= 0x");
  ...
  if (status != ESP_OK) {
    Serial.print("esp_bluedroid_disable status= 0x");
    Serial.println(status, HEX);
    return false;
  }
  ...
  if (status != ESP_OK) {
    Serial.print("esp_bluedroid_deinit status= 0x");
    Serial.println(status, HEX);
    return false;
  }
  ...
  if (status != ESP_OK) {
    Serial.print("esp_bt_controller_disable status= 0x");
    Serial.println(status, HEX);
    return false;
  }
  ...
  if (status != ESP_OK) {
    Serial.print("esp_bt_controller_deinit status= 0x");
    Serial.println(status, HEX);
    return false;
  }
  ...
  if (status != ESP_OK) {
    Serial.print("esp_bt_controller_mem_release status= 0x");
    Serial.println(status, HEX);
    return false;
  }
